...
export class DeckManageController {
    ...
    async open(deckName, allWords) {
        ...
        if (deckName === 'mistake-notebook') {
             this.titleElement.textContent = `管理错题本`;
             const mistakeWords = await this.mistakeRepository.getAllWords();
             // We need full word objects.
             // Filter allWords to find those in mistakeWords list.
             // Note: VocabularyRepository.getWordsByArabic handles fetching from DB,
             // but here we have 'allWords' passed from App (which are loaded into memory).
             // Using 'allWords' is faster if they are up to date.
             const mistakeSet = new Set(mistakeWords);
             this.currentWords = allWords.filter(w => mistakeSet.has(w.arabic));

             if (this.currentWords.length < mistakeWords.length) {
                 // If some words are missing from memory (rare case if allWords is complete),
                 // fallback to repo fetch? But App passes full vocabulary.
                 // If words were deleted from DB but still in mistake repo (orphans), clean them up?
